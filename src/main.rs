...
"#;
    ...
    fn advance_paste_queue(&mut self) {
        ...
        self.refresh_entries_or_status();
        if self.paste_failed_items == 0 {
            self.set_status(format!("paste complete: {} item(s)", self.paste_ok_items));
        } else {
            self.set_status(format!(
                "paste finished: {} ok, {} failed ({} total)",
    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            let status = Line::from(vec![
                ...
                Span::styled("Tab:path h:help i:integrations n:new-file N:new-dir Z:archive C:delta o:open-gui ~:home F2:rename/bulk c:copy v:paste d:del q:quit", Style::default().fg(Color::DarkGray)),
            ]);
